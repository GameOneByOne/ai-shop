...
import { DistributionLogSyncButton } from "@/components/listing/distribution-log-sync-button";
...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const isWarehoused = (product: ProductRow) => { const listing = (traceOf(product.notes).listing as Record<string, unknown> | undefined); return String(listing?.officialStatus) === "SUCCESS" || (["WAREHOUSED", "LISTED"].includes(String(listing?.status)) && typeof listing?.taobaoItemId === "string" && Boolean(listing.taobaoItemId)); };
  ...
  return <>
    ...
    <div className="product-manager-toolbar"><div className="v2-filter-group offer-status-tabs"><Link className={stage === "draft" ? "v2-primary" : "v2-secondary"} href="/products/manage">待铺货 <b>{realProducts.length - warehousedCount}</b></Link><Link className={stage === "warehouse" ? "v2-primary" : "v2-secondary"} href="/products/manage?stage=warehouse">仓库中 <b>{warehousedCount}</b></Link></div><div className="action-buttons"><DistributionLogSyncButton />{stage === "draft" && products.length > 0 && <BatchPublishButton products={products.map((product) => ({ id: product.id, name: product.name }))} />}</div></div>
    {!products.length ? <section className="v2-workbench-empty"><div className="v2-empty-copy"><div><h2>{stage === "warehouse" ? "仓库中还没有已核验商品" : "还没有待铺货商品"}</h2><p>{stage === "warehouse" ? "点击“同步官方铺货日志”，仅把官方成功且带淘宝商品 ID 的记录收入仓库。" : "在货源发现中选择一个 Offer，系统会自动在这里建立商品。"}</p></div></div>{stage === "warehouse" ? <DistributionLogSyncButton /> : <Link className="v2-primary" href="/products/discover">前往货源发现</Link>}</section> : <div className="candidate-pipeline">{products.map((product) => {
      ...
      const warehoused = String(listing?.officialStatus) === "SUCCESS" || (["WAREHOUSED", "LISTED"].includes(String(listing?.status)) && typeof listing?.taobaoItemId === "string" && Boolean(listing.taobaoItemId));
      ...
      return <article className={`card product-work-card${focused === product.id ? " active" : ""}`} key={product.id}>
        ...
        <div className="product-card-footer"><div className="candidate-facts"><div><span>货源编号</span><b>{offerId ?? "已绑定"}</b></div><div><span>采购价</span><b>{product.estimated_cost == null ? "—" : `¥${Number(product.estimated_cost).toFixed(2)}`}</b></div><div><span>淘宝状态</span><b>{warehoused ? "官方铺货成功" : failed ? "官方失败 / 待重试" : awaitingExternalPublish ? "等待官方确认" : "未执行"}</b></div></div><div className="action-buttons">{sourceUrl && <a className="secondary-btn" href={sourceUrl} target="_blank" rel="noreferrer">查看货源</a>}{warehoused && typeof listing?.taobaoItemUrl === "string" ? <a className="btn button-link" href={listing.taobaoItemUrl} target="_blank" rel="noreferrer">打开淘宝商品</a> : <Link className="btn button-link" href={`/content?product=${product.id}`}>{warehoused ? "查看铺货结果" : awaitingExternalPublish ? "立即铺货" : failed ? "重新铺货" : "开始铺货"}</Link>}</div></div>
      </article>;
    })}</div>}
  </>;
}
