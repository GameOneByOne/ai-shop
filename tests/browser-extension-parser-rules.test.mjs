...
const publishSource = readFileSync(new URL("../browser-extension-publish/page-scraper.js", import.meta.url), "utf8");
const publishBackground = readFileSync(new URL("../browser-extension-publish/background.js", import.meta.url), "utf8");
const publishBridge = readFileSync(new URL("../browser-extension-publish/bridge.js", import.meta.url), "utf8");
const materialBackground = readFileSync(new URL("../browser-extension-material/background.js", import.meta.url), "utf8");
const materialBridge = readFileSync(new URL("../browser-extension-material/bridge.js", import.meta.url), "utf8");
...
const publishManifest = JSON.parse(readFileSync(new URL("../browser-extension-publish/manifest.json", import.meta.url), "utf8"));
const materialManifest = JSON.parse(readFileSync(new URL("../browser-extension-material/manifest.json", import.meta.url), "utf8"));
...
assert.match(enrichRoute, /details:\s*z\.array\(detail\)\.max\(500\)/, "详情批次上限必须与页面目标商品数上限一致，并允许空批次完成收尾");
assert.match(background, /offers\.slice\(0, 500\)/, "扩展详情解析上限必须与页面目标商品数上限一致");
assert.match(materialBackground, /30\\s\*个\?汉字\|60\\s\*字符/, "淘宝新版标题必须支持按30汉字/60字符限制直接定位");
assert.match(publishBackground, /立即铺货\(\?:（单店）\)\?/, "确认店铺必须精确识别“立即铺货（单店）”按钮");
assert.match(publishSource, /立即铺货\(\?:\[（\(\]单店\[）\)\]\)\?/, "页面脚本必须识别全角或半角的“立即铺货（单店）”按钮");
assert.match(publishBackground, /确认店铺抽屉仍停留在提交前状态，本次不记录为成功/, "铺货按钮未触发状态变化时禁止返回成功");
assert.doesNotMatch(publishBackground, /recentlyActivated|aiPublishPage/, "铺货结果不得复用铺货前已打开或仅被激活的淘宝草稿页");
assert.match(materialBackground, /const allControls = deep/, "标题控件已有值时不得因暂时不可见而丢弃");
assert.match(materialBackground, /扩展所在浏览器中没有找到已打开的淘宝草稿页/, "跨浏览器打开淘宝页时必须返回明确诊断");
assert.match(materialBridge, /function postMaterial[\s\S]*extensionVersion/, "淘宝草稿错误必须返回当前扩展版本");
...
assert.doesNotMatch(selectionCenter, /<th>分类<\/th>/, "候选表不得保留与AI商品分类重复的旧采集分类列");
assert.match(selectionCenter, /<th>标准商品名<\/th>/, "候选表必须展示第一阶段标准商品名");
...
assert.match(background, /chrome\.tabs\.update\(tab\.id, \{ active: true \}\)/, "复用1688搜索页时必须自动切换到对应标签");
assert.match(background, /chrome\.tabs\.create\(\{ url: offer\.sourceUrl, active: true \}\)/, "详情解析必须真实打开并切换到货源标签页");
...
assert.match(source, /nextPageNumber/, "分页必须支持按当前页加一定位纯数字页码按钮");
assert.match(source, /展开筛选/, "搜索筛选区域闭合时必须支持自动展开");
...
assert.match(background, /第 \$\{page\} 页获取完成/, "搜索列表阶段必须使用获取文案，不能误称为详情解析");
...
assert.match(bridge, /AI_SHOP_CAPTURE_CHECKPOINT/, "详情解析必须支持分批断点保存");
assert.match(background, /pendingCheckpoint\.length >= 10/, "详情解析必须每10条提交一次保存点");
assert.match(enrichRoute, /checkpointedOffers/, "详情写入接口必须区分中间批次与最终完成");
assert.match(enrichRoute, /解析阶段不能标记为完成/, "0条详情不得把解析阶段错误标记为完成");
assert.match(sourcingDiscovery, /checkpointedCount <= 0/, "页面不得提交0条详情的完成请求");
...
assert.match(selectionCenter, />AI选择货源<|"AI选择货源"/, "现有货源页必须提供第一阶段AI货源选择按钮");
assert.match(selectionCenter, /AI选择货源（需先完成解析）/, "详情未解析完成时AI按钮必须保留并说明禁用原因");
assert.match(selectionCenter, /detailedOffers === 0 \? "未执行"/, "详情解析为0条时进度必须明确显示未执行");
assert.match(selectionCenter, />取消AI选择</, "AI货源选择运行中必须提供取消按钮");
assert.match(aiSelectionRoute, /AbortSignal\.any\(\[request\.signal/, "取消AI货源选择必须中止服务端模型请求");
assert.match(aiSelectionRoute, /standardProductName/, "第一阶段AI必须返回标准商品名");
assert.match(aiSelectionRoute, /categoryParent.*categoryChild/, "第一阶段AI必须返回商品内容分类");
assert.match(aiSelectionRoute, /禁止生成淘宝展示标题/, "第一阶段提示词必须禁止生成上架展示标题和营销内容");
assert.match(selectionCenter, /人工批量审批/, "AI推荐与可用货源必须由人工批量审批");
assert.doesNotMatch(selectionCenter, />运行规则</, "规则筛选不得再作为独立操作按钮");
assert.match(selectionCenter, />应用规则</, "AI选择入口必须提供应用规则分支");
assert.match(selectionCenter, />不应用规则</, "AI选择入口必须提供跳过规则分支");
assert.match(selectionCenter, /passedOfferIds/, "应用规则后必须只把规则通过项送入AI评估");
...
assert.match(selectionCenter, /AI_SHOP_CAPTURE_CHECKPOINT/, "结果页重新解析必须接收分批保存点");
assert.match(selectionCenter, /reparse-live-progress/, "结果页重新解析必须展示实时进度条");
...
assert.match(selectionCenter, /failedOffers\s*=\s*offers\.filter\(actionableReparseRequired\)/, "批量重新解析必须与待补数据页签使用相同口径");
assert.match(selectionCenter, /const isUndecided[\s\S]*!isDataPending\(offer\)[\s\S]*Boolean\(offerAiSelection\(offer\)\)/, "待补数据或尚未完成AI分析的货源不得计入待人工决定");
assert.match(selectionCenter, /isDataPending\(selected\)[\s\S]*重新解析缺失项/, "待补数据货源必须引导重新解析，不能直接选择货源");
assert.match(selectionCenter, /const isDataPending = reparseRequired/, "待补数据必须包含尚未解析详情的货源，不能依赖规则是否已运行");
assert.match(selectionCenter, /return ruleDecisionOf\(offer\) !== "REJECTED" \|\|/, "尚未运行规则或待补数据不得误显示为规则淘汰");
...
assert.match(publishSource, /async function startDefaultPublish/, "铺货扩展必须提供默认模板自动铺货流程");
assert.match(publishSource, /clicked_immediate_publish/, "自动铺货必须先点击立即铺货");
assert.match(publishSource, /checked_default_store/, "自动铺货必须勾选默认淘宝店铺");
assert.match(publishSource, /confirmed_publish/, "自动铺货必须点击确定铺货");
assert.match(publishSource, /START_DEFAULT_PUBLISH/, "详情页必须接收默认铺货指令");
assert.match(publishBridge, /AI_SHOP_PUBLISH_1688/, "本地应用必须能通过独立铺货扩展发起任务");
assert.match(publishBackground, /async function publishDefaultListing/, "铺货后台必须编排完整铺货任务");
assert.match(publishBackground, /waitForTaobaoPublishResult/, "铺货完成后必须识别淘宝商品ID");
assert.match(materialBackground, /captureAttempt < 12/, "淘宝草稿读取必须等待新版发布页异步渲染标题控件");
assert.match(materialBackground, /商品标题.*商品名称/, "淘宝草稿标题读取必须兼容新版字段名称");
assert.match(materialBackground, /control\.getAttribute\("data-testid"\)/, "淘宝草稿标题读取必须支持新版控件标识");
assert.equal(manifest.name, "AI 店长 1688 货源采集");
assert.equal(publishManifest.name, "AI 店长 1688 一键铺货");
assert.equal(materialManifest.name, "AI 店长淘宝商品素材管理");
assert.doesNotMatch(bridge, /AI_SHOP_PUBLISH_1688|AI_SHOP_AUTOFILL_TAOBAO|AI_SHOP_CAPTURE_TAOBAO_DRAFT/, "采集扩展不得响应铺货或淘宝素材消息");
assert.doesNotMatch(publishBridge, /AI_SHOP_CAPTURE_1688|AI_SHOP_ENRICH_1688|AI_SHOP_AUTOFILL_TAOBAO|AI_SHOP_CAPTURE_TAOBAO_DRAFT/, "铺货扩展不得响应采集或淘宝素材消息");
assert.doesNotMatch(materialBridge, /AI_SHOP_CAPTURE_1688|AI_SHOP_ENRICH_1688|AI_SHOP_PUBLISH_1688/, "素材扩展不得响应采集或铺货消息");
assert.equal(manifest.host_permissions.some((value) => value.includes("taobao.com")), false, "采集扩展不得申请淘宝权限");
assert.equal(materialManifest.host_permissions.some((value) => value.includes("1688.com")), false, "素材扩展不得申请1688权限");
assert.match(selectionCenter, /尚未运行本轮「AI评估货源」/, "AI评估Tab只能展示已保存结果或空状态");
assert.match(aiSelectionRoute, /offer-source-evaluation-v6-structured-recognition-evaluation/, "AI评估必须使用结构化商品识别与货源评估版本");
...
assert.match(sourcingDiscovery, /extensionVersion:\s*event\.data\.extensionVersion/, "货源任务必须保存实际执行时的扩展版本");
assert.match(importRoute, /extensionVersion:\s*parsed\.data\.extensionVersion/, "导入任务必须持久化扩展版本");
